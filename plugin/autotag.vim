...
class AutoTag:
    __maxTagsFileSize = 1024 * 1024 * 10
    __threshold = 1
    ...
    def __init__(self):
        ...
        self.count = 0
        self.use_relative_path = long(vim.eval("g:autotagUseRelativePath"))
    ...
    def addSource(self, source):
        ...
        if suffix in self.excludesuffix:
            self.__diag("Ignoring excluded suffix %s for file %s" % (source, suffix))
            return
        tagsFile = self.findTagFile(source)
        if tagsFile:
            this_source = source
            if self.use_relative_path > 0:
                this_source = source[len(os.path.dirname(tagsFile)):]
                if this_source[0] == os.sep:
                    this_source = this_source[1:]
            if os.sep != self.sep_used_by_ctags:
                this_source = string.replace(this_source, os.sep, self.sep_used_by_ctags)
            if self.tags.has_key(tagsFile):
                self.tags[tagsFile].append(this_source)
            else:
                self.tags[tagsFile] = [ this_source ]
...
" [TODO]( <zhaocai> 2011-12-25 12:13PM ) more patterns to ignore
...
if !exists("g:autotagUseRelativePath")
   let g:autotagUseRelativePath=0
endif
augroup autotag
   au!
